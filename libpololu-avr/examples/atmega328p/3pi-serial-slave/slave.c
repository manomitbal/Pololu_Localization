...
void sonarDistance()
{
  ...
  start_analog_conversion(7);  // start initial conversion
  if (!analog_is_converting())     // if conversion is done...
  {
    result = (float)(25.0 * analog_conversion_result() / 5.12 / 10.0);  // get result
    message[0] = result;
  }
  serial_send((char *)message, 2);
} 

void rxsonarDistance(int flag)
{

	time_reset();
	unsigned long begin, end, difference;
	//begin = get_ms();
	while(1)
	{
	...
	//	if(flag*1000 <= difference)
	//		break;
	//	else
	//	{
			unsigned int result[1];
			result[0] = analog_read(7);
			serial_send_blocking((char *)result, 1);
			delay_ms(5);
	//	}
	}
...
int main()
{
	...
	pololu_3pi_init_disable_emitter_pin(1);
